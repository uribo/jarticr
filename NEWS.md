...
* Added an offline-testable client for the four xROAD traffic-volume API layers, with typed CQL construction, separate HTTP execution, GeoJSON and JSON-encoded CSV parsing, three layer schemas, typed `data.table` results, three-state CCTV flags, body-based API failure detection, transient-only retries, and request throttling. Recorded probe responses pin the deployed property names, HTTP 200 payload-cap error, invalid quoted-CQL response, empty-result ambiguity, CSV coordinate precision, and form 4's two-digit `time_slot`, 12-digit request `time_code`, and observed `"0"`/`"2"` processing flags.
